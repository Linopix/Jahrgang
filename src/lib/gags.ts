...
export function noteKonamiKey(key: string) {
  ...
  if (match) {
    ...
    konamiTimer = window.setTimeout(() => {
      const progress = konami;
      konami = 0;
      roastKonami(progress);
    }, 1500);
    return;
  }
  const progress = konami;
  konami = key === KONAMI[0] ? 1 : 0;
  if (progress > 0) roastKonami(progress);
  else if (KONAMI_ARROWS.has(key)) roastKonami(1);
  if (konami === 1) {
    konamiTimer = window.setTimeout(() => {
      konami = 0;
      roastKonami(1);
...
function roastKonami(progress: number) {
  const now = Date.now();
  if (now - lastKonamiRoast < 2200) return;
  lastKonamiRoast = now;
  if (progress <= 1) {
    toastGag(KONAMI_ONE[Math.floor(Math.random() * KONAMI_ONE.length)] ?? KONAMI_ONE[0]);
    return;
  }
  toastGag(KONAMI_NEAR[(progress - 2) % KONAMI_NEAR.length] ?? "Fast.");
}
